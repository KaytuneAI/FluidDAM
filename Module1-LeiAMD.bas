...
' Enhanced border detection with multiple methods
Private Function HasCellBorder(ByVal cell As Range) As Boolean
    On Error Resume Next
    HasCellBorder = False
    
    ' 方法1: 使用 DisplayFormat（包括条件格式）
    Dim hasTop As Boolean, hasBottom As Boolean, hasLeft As Boolean, hasRight As Boolean
    Dim hasTopDisplay As Boolean, hasBottomDisplay As Boolean, hasLeftDisplay As Boolean, hasRightDisplay As Boolean
    
    hasTopDisplay = (cell.DisplayFormat.Borders(xlEdgeTop).lineStyle <> xlLineStyleNone)
    hasBottomDisplay = (cell.DisplayFormat.Borders(xlEdgeBottom).lineStyle <> xlLineStyleNone)
    hasLeftDisplay = (cell.DisplayFormat.Borders(xlEdgeLeft).lineStyle <> xlLineStyleNone)
    hasRightDisplay = (cell.DisplayFormat.Borders(xlEdgeRight).lineStyle <> xlLineStyleNone)
    
    ' 方法2: 使用普通的Borders
    Dim hasTopRegular As Boolean, hasBottomRegular As Boolean, hasLeftRegular As Boolean, hasRightRegular As Boolean
    
    hasTopRegular = (cell.Borders(xlEdgeTop).lineStyle <> xlLineStyleNone)
    hasBottomRegular = (cell.Borders(xlEdgeBottom).lineStyle <> xlLineStyleNone)
    hasLeftRegular = (cell.Borders(xlEdgeLeft).lineStyle <> xlLineStyleNone)
    hasRightRegular = (cell.Borders(xlEdgeRight).lineStyle <> xlLineStyleNone)
    
    ' 方法3: 检查表格样式边框
    Dim hasTableBorder As Boolean
    hasTableBorder = HasTableStyleBorder(cell)
    
    ' 方法4: 检查条件格式边框
    Dim hasConditionalBorder As Boolean
    hasConditionalBorder = HasConditionalFormatBorder(cell)
    
    ' 方法5: 检查相邻单元格的边框（可能影响当前单元格的显示）
    Dim hasAdjacentBorder As Boolean
    hasAdjacentBorder = HasAdjacentCellBorder(cell)
    
    ' 综合判断：任何一个方法检测到边框都认为有边框
    hasTop = hasTopDisplay Or hasTopRegular
    hasBottom = hasBottomDisplay Or hasBottomRegular
    hasLeft = hasLeftDisplay Or hasLeftRegular
    hasRight = hasRightDisplay Or hasRightRegular
    
    HasCellBorder = hasTop Or hasBottom Or hasLeft Or hasRight Or hasTableBorder Or hasConditionalBorder Or hasAdjacentBorder
    
    On Error GoTo 0
End Function

' 检查表格样式边框
Private Function HasTableStyleBorder(ByVal cell As Range) As Boolean
    On Error Resume Next
    HasTableStyleBorder = False
    
    ' 检查是否在表格中
    If Not cell.ListObject Is Nothing Then
        ' 检查表格边框样式
        Dim tableStyle As String
        tableStyle = cell.ListObject.TableStyle
        If tableStyle <> "" And tableStyle <> "None" Then
            HasTableStyleBorder = True
        End If
    End If
...
' 检查条件格式边框
Private Function HasConditionalFormatBorder(ByVal cell As Range) As Boolean
    On Error Resume Next
    HasConditionalFormatBorder = False
    
    ' 检查条件格式
    Dim cfCount As Long
    cfCount = cell.FormatConditions.Count
    
    Dim i As Long
    For i = 1 To cfCount
        Dim cf As FormatCondition
        Set cf = cell.FormatConditions(i)
        
        ' 检查条件格式是否包含边框设置
        If cf.Borders.LineStyle <> xlLineStyleNone Then
            HasConditionalFormatBorder = True
            Exit Function
        End If
    Next i
...
' 检查相邻单元格的边框影响
Private Function HasAdjacentCellBorder(ByVal cell As Range) As Boolean
    On Error Resume Next
    HasAdjacentCellBorder = False
    
    ' 检查上方单元格的下边框
    If cell.Row > 1 Then
        Dim cellAbove As Range
        Set cellAbove = cell.Offset(-1, 0)
        If cellAbove.Borders(xlEdgeBottom).lineStyle <> xlLineStyleNone Or _
           cellAbove.DisplayFormat.Borders(xlEdgeBottom).lineStyle <> xlLineStyleNone Then
            HasAdjacentCellBorder = True
            Exit Function
        End If
    End If
    
    ' 检查下方单元格的上边框
    If cell.Row < cell.Worksheet.Rows.Count Then
        Dim cellBelow As Range
        Set cellBelow = cell.Offset(1, 0)
        If cellBelow.Borders(xlEdgeTop).lineStyle <> xlLineStyleNone Or _
           cellBelow.DisplayFormat.Borders(xlEdgeTop).lineStyle <> xlLineStyleNone Then
            HasAdjacentCellBorder = True
            Exit Function
        End If
    End If
    ...
    If cell.Column > 1 Then
        ...
        Set cellLeft = cell.Offset(0, -1)
        If cellLeft.Borders(xlEdgeRight).lineStyle <> xlLineStyleNone Or _
           cellLeft.DisplayFormat.Borders(xlEdgeRight).lineStyle <> xlLineStyleNone Then
            HasAdjacentCellBorder = True
            Exit Function
        End If
    End If
    ...
    ' 检查右侧单元格的左边框
    If cell.Column < cell.Worksheet.Columns.Count Then
        Dim cellRight As Range
        Set cellRight = cell.Offset(0, 1)
        If cellRight.Borders(xlEdgeLeft).lineStyle <> xlLineStyleNone Or _
           cellRight.DisplayFormat.Borders(xlEdgeLeft).lineStyle <> xlLineStyleNone Then
            HasAdjacentCellBorder = True
            Exit Function
        End If
    End If
