...
export class ExcelToTLDrawConverter {
  ...
  async convertExcelToTLDraw(file) {
    try {
      ...
      // 验证编辑器
      const editorCheck = validateEditor(this.editor);
      if (!editorCheck.valid) {
        return createError(editorCheck.error);
      }
...
export async function importExcelToTLDraw(file, editor) {
  ...
  try {
    ...
    if (result.success) {
      return createSuccess('Excel导入成功', {
        shapesCount: result.data.shapesCount || 0,
        message: result.message || 'Excel导入成功'
      });
    } else {
      return createError(result.error || '导入失败', result.code, result.details);
    }
  } catch (error) {
    console.error('导入Excel时出错:', error);
    return createError(error.message || '导入过程中发生未知错误', 'IMPORT_ERROR', { stack: error.stack });
  }
}
